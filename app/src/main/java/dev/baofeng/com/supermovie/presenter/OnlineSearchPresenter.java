...
public class OnlineSearchPresenter extends BasePresenter<IOnlineSearch> {
    ...
    public void searchOnline(String keywords) {
        ...
        BaseApi.request(onlineSearch, new BaseApi.IResponseListener<OnlinePlayInfo>() {
                    @Override
                    ...
                    //在线电影请求接口完成，马上请求在线剧集接口。
                    private void onComplete() {
                        BaseApi.request(onlineSearchSeris, new BaseApi.IResponseListener<OnlinePlayInfo>() {
                                @Override
                                public void onSuccess(OnlinePlayInfo result) {
                                    dataBeans.addAll(result.getData());
                                    iview.loadData(info);
